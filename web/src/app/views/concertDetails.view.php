                ...
                <div class="overflow-hidden border-b border-gray-200 shadow sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <th scope="col"
                                class="px-6 py-3 text-xs font-medium tracking-wider text-center text-blue-100 uppercase">
                                Nombre de participants au concert
                            </th>
                            <th scope="col"
                                class="px-6 py-3 text-xs font-medium tracking-wider text-center text-blue-100 uppercase">
                        ...
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="px-6 py-4 text-sm text-center text-gray-500 whitespace-nowrap">
                                <?php if (is_null($data['concert'][0]->moyennenotes)) {
                                    ...
                                    echo(number_format($data['concert'][0]->moyennenotes, 2) . '/5');
                                }
                                ?>
                            </td>
                        </tr>
...
<?php if (isset($_SESSION['id'])) : ?>
    <div class="flex justify-center text-center">
        <form action="/noteConcert" method="POST">
            <input class="" value="<?= $data['concert'][0]->id ?>" type="hidden" id="idConcert" name="idConcert"
                   required>
    ...
    <div class="block pr-4 text-sm text-center text-indigo-500">Vous devez avoir assisté à ce concert afin de pouvoir le
        noter.
    </div>
<?php endif; ?>
    ...
    <div class="flex justify-center text-center">
        <form action="/deleteConcert" method="POST">
            <input class="" value="<?= $data['concert'][0]->id ?>" type="hidden" id="idConcert" name="idConcert"
                   required>
            <button type="submit"
                    class="justify-center px-4 py-2 mt-6 ml-4 font-bold text-white bg-red-500 rounded shadow hover:shadow-xl focus:shadow-outline focus:outline-none">
    ...
    <div class="flex justify-center text-center">
        <form action="/signup" method="POST">
            ...
            <input class="" value="<?= $data['concert'][0]->id ?>" type="hidden" id="idConcert" name="idConcert"
                   required>
            <button type="submit"
                    class="justify-center px-4 py-2 mt-6 ml-4 font-bold text-indigo-100 bg-blue-900 rounded shadow hover:shadow-xl focus:shadow-outline focus:outline-none">
...
<?php endif; ?>
    ...
    <h1 class="mt-16 text-5xl text-center text-blue-400">Le créateur du concert
        est <?= $data['concert'][0]->prénom . ' ' . $data['concert'][0]->nomuser ?></h1>
    <h1 class="mt-6 text-xl text-center text-blue-800 hover:underline"><a
                href="/user?id=<?= $data['concert'][0]->idcréateur ?>">Cliquez
            ici pour voir son profil</a></h1>
